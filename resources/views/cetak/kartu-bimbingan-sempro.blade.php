...
<head>
    <meta charset="UTF-8">
    <title>Kartu Bimbingan Seminar Proposal - {{ $sempro->riwayatPendidikan?->siswa?->nama }}</title>
    <style>
        @page {
            margin: 0.5cm;
        }
...
<body>
    ...
    @php
    $pembimbings = [];
    if ($sempro->id_dosen_pembimbing_1) $pembimbings[] = 1;
    if ($sempro->id_dosen_pembimbing_2) $pembimbings[] = 2;
    if ($sempro->id_dosen_pembimbing_3) $pembimbings[] = 3;
    ...
    $historyHtml = $sempro->$hField;

    // Gunakan delimiter # agar tidak bentrok dengan karakter /
    ...
    <div class="card-section {{ ($idx + 1 == $totalP) ? 'last-page' : '' }}">
        ...
        <table class="consult-table">
            ...
            <tbody>
                ...
                $content = $entry[3]; // Keep HTML for rich content but might need cleaning

                // Extract just the date part for the Date column
                ...
                {{-- Add empty rows to match the physical card look --}}
