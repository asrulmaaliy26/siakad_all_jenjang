...
<head>
    ...
    <title>Cetak Absensi Terisi - {{ $kelas->mataPelajaranKurikulum->mataPelajaranMaster->nama ?? 'Mata Pelajaran' }}</title>
    <style>
        body {
...
<body onload="window.print()">
    ...
    <table class="data">
        <thead>
            ...
            <tr>
                @for($i = 1; $i <= 12; $i++)
                    <th style="width: 20px; font-size: 9px;">
                    @if(isset($sesiAbsensi[$i-1]))
                    {{ \Carbon\Carbon::parse($sesiAbsensi[$i-1]->tanggal)->format('d/m') }}
                    @else
                    {{ $i }}
                    @endif
                    </th>
        ...
        <tbody>
            ...
            <tr>
                ...
                @for($i = 1; $i <= 12; $i++)
                    ...
                    @if(isset($sesiAbsensi[$i-1]))
                    @php
                    $status = $absensiData->where('id_krs', $krs->id)->filter(function($item) use ($sesiAbsensi, $i) {
                    return \Carbon\Carbon::parse($item->waktu_absen)->format('Y-m-d') == $sesiAbsensi[$i-1]->tanggal;
                    })->first();
                    @endphp
                    ...
                    H
                    @elseif($status->status == 'Izin')
                    I
                    @elseif($status->status == 'Sakit')
                    S
                    @elseif($status->status == 'Alpa')
                    A
                    @endif
                    @endif
                    @endif
                    ...
                    @endfor
                    @php
                    $hadir = $absensiData->where('id_krs', $krs->id)->where('status', 'Hadir')->count();
                    $total = $sesiAbsensi->count();
                    $percent = $total > 0 ? ($hadir / $total) * 100 : 0;
                    @endphp
                    <td style="font-size: 9px;">{{ $total > 0 ? round($percent).'%' : '' }}</td>
            </tr>
            @empty
            <tr>
